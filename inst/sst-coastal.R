...
dens <- tbl(db, "sst_density_tab") %>% dplyr::filter(Zone == "High-Latitude")
...
ggplot(d %>% dplyr::filter(depth >= -250),  aes(x = max, group = decade, weight = 25, colour = decade)) + 
  geom_density() + facet_wrap(SectorName ~ season) 

ggplot(d %>% dplyr::filter(depth >= -100), aes(x = max, group = decade, weight = 25, colour = decade)) + 
  geom_density() + facet_wrap(SectorName ~ season) 
